...
import {ROUTE,SIGNAL,GameMode,GetVersion, GetPlatform, PLATFORM, GetCFrom} from './Constants.js'
...
class GameSocket {
    ...
    get _isH5 () {
        return this._platform === PLATFORM.kH5 ? 1 : 0;
    }
    ...
    _connectForLogin(params,callback){
        ...
        const getGateServer = function () {
            return new Promise(function (resolve,reject) {
                if (status !== self._status) return;
                self.socket.request(ROUTE.GET_GATE_SERVER,{appVersion: self._version, requestZone: 'Mainland', game: self._cFrom, isH5: self._isH5}, function (response) {
                ...
                self.socket.request(ROUTE.GET_CONNECTOR,{},function (response) {
                    ...
                    if(response && response.code === SIGNAL.OK){
                        ...
                        if (!self.didConnectForSignIn) {
                            ...
                            self._connectForSignIn(params, callback);
                        }
                    } else {
                        const msg = response.msg || 'unknown err';
                        const err = {};
        ...
        const exceptionFunc = function (err) {
            const msg = "name: " + err.name + "message: " + err.message + "stack: " + JSON.stringify(err.stack);
